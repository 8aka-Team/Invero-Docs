---
sidebar_position: 2
---
# Installation

Download the latest version of Invero from [here](https://invero.8aka.org/download)

<!-- [![](https://img.shields.io/badge/QQ群-489868834-54B4EF?logo=data:image/png;base64,)](https://8aka.org/qq) <-点击加入 -->

---

### Compatibility

The following server types have been tested and confirmed to be compatible

| Server Type   | Compatibility |
|---------------|---------------|
| Spigot        | ✅             |
| Paper         | ✅             |
| Purpur        | ✅             |
| Catserver     | ✅             |
| Arclight      | ⭕             |
| Mohist        | ⭕             |

:::tip Version
We support Minecraft versions 1.8 - 1.21
:::

:::caution Note
Invero is designed for Bukkit servers. We guarantee compatibility with the latest Spigot/Paper and will **try our best** to be compatible with their related forks
Other platforms are not supported by default
:::

---

### Installing the Plugin

Place the plugin jar file in your server's plugins directory and restart the server
You should see the following information:

```
[I] Successfully connected to SQLITE database
[I] Initializing 1 workspace... Starting to load menus
[I] Successfully loaded 1 valid menu... (0.538 secs)
```

If no Invero-related error messages appear during startup, the plugin has been installed successfully

---


### About Dependencies

Invero is a complete, standalone plugin that does not require any **third-party plugins** as dependencies

Invero needs internet connection during startup to download dependencies from Alibaba Cloud Central Repository. All dependencies are well-known open source projects
Downloaded dependencies are cached in the server's `libs` root directory

:::info
Invero's startup and full plugin functionality require an internet connection
:::
